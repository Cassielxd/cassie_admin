<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      <el-form-item prop="dict_name" :label="$t('dict.dictName')">
        <el-input v-model="dataForm.dict_name" :placeholder="$t('dict.dictName')"></el-input>
      </el-form-item>
      <el-form-item prop="dict_type" :label="$t('dict.dictType')">
        <el-input v-model="dataForm.dict_type" :placeholder="$t('dict.dictType')"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/dict/type/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code != 0) {
